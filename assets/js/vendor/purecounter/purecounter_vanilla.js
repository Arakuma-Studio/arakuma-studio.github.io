...
function formatNumber(number, config) {
    ...
    var isLessThanThousand = number < 1000;

    // Now format the number to string based on its locale.
    number = number.toLocaleString(locale, strConfig);
    
    if (isLessThanThousand) number = number.replace(",",".");
    
    var symbol = config.currency && typeof config.currency === "string"
                    ? config.currency : "";
    
    var plusSymbol = config.plus ? "+" : "";
    
    var stringNum = symbol + applySeparator(number, config) + plusSymbol;

    if (isLessThanThousand) stringNum = stringNum.replace(",",".");
...
function PureCounter(options = {}) {
    var configs = {
        ...
        decimals: 2, // Decimal places [uint]
        legacy: true, // If this is true it will use the scroll event listener on browsers
        filesizing: false, // Enable filesizing counting [boolean]
        currency: false, // Is it for currency?
        plus: false,
        separator: true, // Do you want to use thausands separator? use it for set the symbol too [boolean|char|string]
        formater: "en-US", // Number toLocaleString locale/formater, by default is "en-US" [string|boolean:false]
